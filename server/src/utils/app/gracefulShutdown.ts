...
export const gracefulShutdown = (signal: string, server: http.Server, timeoutMs: number) => (error?: unknown) => {
  if (isShuttingDown) {
    console.warn(`Graceful shutdown already in progress, ignoring ${signal}`);
    return;
  }
  isShuttingDown = true;

  if (!!error) {
    console.error(`[${signal}] Shutdown due to error:`, error);
  } else {
  ...
  const timer = setTimeout(() => {
    console.error(`Forced shutdown after ${timeoutMs}ms timeout`);
