...
export const calcArtComplexity = (art: D.Level.Grid): number => {
  let complexity = 0;

  art.forEach((row, rowIdx) => {
    row.forEach((col, colIdx) => {
      const rightCell: D.Level.Cell | undefined = row[colIdx + 1];
      const bottomCell: D.Level.Cell | undefined = art[rowIdx + 1]?.[colIdx];
      const diagonalCell: D.Level.Cell | undefined = art[rowIdx + 1]?.[colIdx + 1];
...
const compareCells = (a: D.Level.Cell, b: D.Level.Cell): number => {
